...
def check_references() -> None:
    ...
    if not manifest.exists():
        warnings.append("references/manifest.json is missing; provenance cannot be checked.")
        return
    data = json.loads(manifest.read_text(encoding="utf-8"))
    for page in data.get("pages", []):
        if not (refs / f"{page['slug']}.md").exists():
            errors.append(f"manifest lists {page['slug']}.md but the file is missing.")
    # Provenance is the skill's core safety property; a reference without a source
    # comment cannot be traced back and should not be trusted.
    for path in refs.glob("*.md"):
        if path.name in {"INDEX.md", "README.md"}:
            continue
        if "<!-- source:" not in path.read_text(encoding="utf-8"):
            errors.append(f"{path.name} has no source provenance comment.")
